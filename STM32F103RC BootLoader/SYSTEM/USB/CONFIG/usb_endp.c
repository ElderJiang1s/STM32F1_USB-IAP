...
void EP1_IN_Callback (void)
{
	...
	{
		if(ss)//如果之前发送的最后一个数据包等于端点最大包长度，就再发送一个长度为0的数据包，让主机认为传输结束
		{
			ss = 0;
			UserToPMABufferCopy(&uu_txfifo.buffer[USB_Tx_ptr], ENDP1_TXADDR, 0);
			SetEPTxCount(ENDP1, 0);
			SetEPTxValid(ENDP1);//发送完成了，发送0长度数据包，防止出现不能发送64整数倍bug
		}
		uu_txfifo.readptr = 0;//发送完成了，清0读写指针
	...
	if(USB_Tx_length == VIRTUAL_COM_PORT_DATA_SIZE)//如果这个数据包长度等于端点最大包长度
	{
		ss = 1;
	}		
...
void EP3_OUT_Callback(void)
{
	...
	usb_data_process(USB_Rx_Buffer, USB_Rx_Cnt);	//处理数据（其实就是保存数据） 
	//USB_Rx_Buffer是usb内部定义的接收缓冲，长度是接收端点wMaxPacketSize
	//要在usb_data_process中把他放到我们自己定义的接收缓冲里
